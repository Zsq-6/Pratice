...
class Tenant(db.Model):
    ...
    # projects = db.relationship('Project', backref=db.backref('tenants', lazy='dynamic'))
...
class Project(db.Model):
    ...
    # 项目所属租户
    # tenant_id = db.Column(db.String(128), db.ForeignKey('tenant.id'))
    createdat = db.Column(db.DateTime(), default=datetime.utcnow)
    updatedat = db.Column(db.DateTime(), default=datetime.utcnow)
#     # 项目相关配置
...
def create_project():
    ...
    project = Project(name=name,describe=describe,puid=puid,createdat=createdat,updatedat=updatedat)
    # print('------------------')
